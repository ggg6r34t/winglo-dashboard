// Analytics — workforce-level performance, leaderboard, distribution, anomalies.

// Smooth area chart for ops over time
const AreaChart = ({ data, w = 600, h = 180, color = "var(--fg-1)" }) => {
  const max = Math.max(...data), min = Math.min(...data);
  const range = max - min || 1;
  const stepX = w / (data.length - 1);
  const pts = data.map((v, i) => [i * stepX, h - ((v - min) / range) * (h - 20) - 10]);
...
const BarChart = ({ data, h = 180 }) => {
  ...
  return (
    <svg width="100%" height={h} viewBox={`0 0 600 ${h}`} preserveAspectRatio="none" style={{ overflow: "visible" }}>
      {data.map((d, i) => {
        const bw = 600 / data.length - 8;
        const x = i * (600 / data.length) + 4;
        const bh = (d.value / max) * (h - 30);
        return (
          <g key={i}>
            <rect x={x} y={h - bh - 18} width={bw} height={bh} rx="2" fill={d.color || "var(--fg-2)"} opacity="0.85" />
...
const AnalyticsPage = () => {
  ...
  // Synthetic data
  const opsTimeline = [128, 142, 134, 156, 170, 168, 184, 192, 178, 196, 210, 224, 232, 218, 240, 256, 248, 268, 284, 272, 290, 312, 304, 318];
  ...
  const leaderboard = [...AGENTS].sort((a, b) => b.runs - a.runs).slice(0, 6);
  ...
  const heatmap = Array.from({ length: 7 }, (_, d) =>
    Array.from({ length: 24 }, (_, h) => {
      ...
      return Math.min(1, business * weekend * (0.5 + Math.random() * 0.6));
    })
  );
        ...
        <div className="chart-head">
          ...
          <div className="dist-legend">
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <span className="swatch" style={{ background: "var(--bg-3)" }}></span>quiet
              <span className="swatch" style={{ background: "oklch(0.55 0.06 75)", marginLeft: 10 }}></span>busy
              <span className="swatch" style={{ background: "var(--accent)", marginLeft: 4 }}></span>peak
            </span>
          </div>
        </div>
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((label, d) => (
            ...
            <div className="heatmap-cells">
              {heatmap[d].map((v, h) => (
                <div
                  ...
                  style={{
                    background: v < 0.2 ? "var(--bg-2)"
                              : v < 0.4 ? "oklch(0.30 0.04 75)"
                              : v < 0.7 ? "oklch(0.50 0.08 75)"
                              : "oklch(0.72 0.12 75)",
                  }}
