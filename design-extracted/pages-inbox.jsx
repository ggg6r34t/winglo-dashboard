// Inbox — threaded messages from AI employees to the operator.
...
const INBOX = [
  ...
  },
  {
    ...
    preview: "Blog, email, X thread, LinkedIn variants are all drafted and read for embargo Thursday 09:00. Approval is in your queue. Sharing here for visibility.",
    time: "2h",
    unread: false,
    ...
    artifacts: [
      { name: "launch-package-v4.zip", meta: "4 files · 240KB", icon: "ZIP" },
    ],
  },
  {
    id: "m-5",
  ...
  },
  {
    ...
    artifacts: [
      { name: "canonical-fix.diff", meta: "11 files · +24 / −24", icon: "DIFF" },
    ],
  },
  {
    id: "m-7",
...
const InboxItem = ({ msg, active, onClick }) => {
  const { AGENT_BY_ID } = window.WingloData;
  const agent = AGENT_BY_ID[msg.from];
  return (
    <div
      className={"inbox-item" + (active ? " active" : "") + (msg.unread ? " unread" : "")}
      onClick={onClick}
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div className="inbox-tags">
          {msg.tags.map(t => <span key={t} className="tag">{t}</span>)}
        </div>
      </div>
    </div>
  );
...
const InboxDetail = ({ msg }) => {
  const { AGENT_BY_ID } = window.WingloData;
  const agent = AGENT_BY_ID[msg.from];
  return (
    <div className="inbox-detail fade-in" key={msg.id}>
      <div className="detail-head">
        ...
        <div style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-3)" }}>
          {msg.time} ago · {msg.id.toUpperCase()}
        </div>
      </div>
      <h1 className="detail-subject">{msg.subject}</h1>
      <div className="detail-body">
        {msg.body.map((p, i) => <p key={i}>{p}</p>)}
...
const InboxPage = () => {
  ...
  const [activeId, setActiveId] = React.useState(INBOX[0].id);
  const filters = [
    { id: "all", label: "All", count: INBOX.length },
    { id: "unread", label: "Unread", count: INBOX.filter(m => m.unread).length },
    { id: "decision", label: "Decisions", count: INBOX.filter(m => m.tags.includes("decision") || m.tags.includes("approval")).length },
    { id: "reports", label: "Reports", count: INBOX.filter(m => m.tags.includes("report")).length },
      ...
      <div className="inbox-list">
        <div className="inbox-filters">
          {filters.map(f => (
            ...
            >
              {f.label}{f.count != null && <span className="count">{f.count}</span>}
            </button>
          ))}
        </div>
        {filtered.map(m => (
