// Audit log — dense, mono, timestamp-first table.
...
const AuditPage = () => {
  const { AGENT_BY_ID, AGENTS } = window.WingloData;
  ...
  return (
    <div className="page fade-in">
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        {AGENTS.map(a => {
          const c = AUDIT_LOG.filter(r => r.agent === a.id).length;
          if (c === 0) return null;
          return (
            <button
              key={a.id}
            ...
            >
              {a.name} <span className="count">{c}</span>
            </button>
          );
        })}
        <span style={{ width: 1, background: "var(--line-1)", margin: "0 4px" }}></span>
        {[
      ...
      <div className="audit-list">
        ...
        {filtered.map((row, i) => {
          const a = AGENT_BY_ID[row.agent];
          return (
            <div className="audit-row" key={i}>
              <div className="audit-time">{row.time}</div>
              <div className="audit-glyph-cell"><AgentGlyph agent={a} size={20} /></div>
      ...
      <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-3)" }}>
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
          <span className="pulse" style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent)" }}></span>
          tail live
        </span>
      </div>
    </div>
