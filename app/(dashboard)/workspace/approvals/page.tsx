    ...
    <div className="page fade-in">
      ...
      <div className="approvals-grid">
        <aside className="approvals-side">
          ...
          {CATEGORIES.map(c => {
            const count = c.id === "all" ? APPROVAL_ITEMS.length : APPROVAL_ITEMS.filter(a => a.type === c.id).length
            return (
              <div
                key={c.id}
          ...
          <div className="side-item">
            <span>High</span>
            <span className="count">{APPROVAL_ITEMS.filter(a => a.urgency === "high").length}</span>
          </div>
          <div className="side-item">
            <span>Medium</span>
            <span className="count">{APPROVAL_ITEMS.filter(a => a.urgency === "med").length}</span>
          </div>
          <div className="side-item">
            <span>Low</span>
            <span className="count">{APPROVAL_ITEMS.filter(a => a.urgency === "low").length}</span>
          </div>
        </aside>
        ...
        <div>
          <div className="approval-row-list">
            ...
            {filtered.map(a => {
              const agent = AGENT_MAP[a.agent]
              return (
                <div className="approval-row" key={a.id}>
                  <PageAgentGlyph agentId={a.agent} size={26} />
                  <div>
