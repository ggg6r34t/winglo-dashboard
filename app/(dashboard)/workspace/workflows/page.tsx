...
import { AGENT_MAP } from "@/components/workspace/page-agent-glyph"

interface WorkflowNode {
  id: string
  type: "start" | "step" | "gate" | "end"
  label: string
  title: string
  x: number
  y: number
...
function WorkflowGraph({ wf }: { wf: Workflow }) {
...
export default function WorkflowsPage() {
  const [activeId, setActiveId] = useState("wf-content")
  const wf = WORKFLOW_LIBRARY.find(w => w.id === activeId)!
  const agent = AGENT_MAP[wf.agent]
              ...
              >
                <div className="wf-list-top">
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div className={"wf-state " + w.state} />
          ...
          <div style={{ background: "var(--bg-1)", border: "1px solid var(--line-1)", borderRadius: "var(--r-lg)", padding: "16px 18px", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 22, marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--line-1)" }}>
              <div>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--fg-0)" }}>{wf.runs}</div>
              </div>
              <div>
                <div className="kpi-label">Success</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--ok)" }}>{(wf.success * 100).toFixed(1)}%</div>
              </div>
              <div>
                <div className="kpi-label">Avg duration</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--fg-0)" }}>4m 18s</div>
              </div>
              <div>
                <div className="kpi-label">Last run</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--fg-0)" }}>14m ago</div>
              </div>
              <div>
                <div className="kpi-label">Next run</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--fg-0)" }}>Mon 06:00</div>
              </div>
            </div>
          </div>
              ...
              <div className="workflow-list">
                {wf.runs_recent.map(r => (
                  <div className="wf-row" key={r.id}>
                    <div className={"wf-state " + r.state} />
